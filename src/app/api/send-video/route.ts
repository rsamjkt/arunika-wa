...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    logEvent({ ownerId, direction: "out", session, chatId, kind: "video", status: "sent", source: "manual" });
    incrementQuotaUsage(ownerId);
    return NextResponse.json(message, { status: 201 });
  } catch (err) {
    const status = err instanceof WahaError ? err.status : 500;
    const errorMessage = err instanceof Error ? err.message : "Unknown error";
    logEvent({ ownerId, direction: "out", session, chatId, kind: "video", status: "failed", source: "manual", error: errorMessage });
